...
public:
    ...
    void DmdAutoSave();
    bool DmdRunOfflineDotCalibration(double* outRms, QString* outMessage);
    bool DmdMapCameraToDmd(double camX, double camY, int* dmdX, int* dmdY) const;
    ...
    void UpdateTestInputStatus();       // 同步右侧测试输入状态。
    void UpdateVirtualDmdPreview();     // 刷新虚拟 DMD 靶点预览图。
    void UpdateTargetAccuracyMonitor(); // 刷新轻量目标精度监控。
...
void on_LedOnOff_clicked();   //Led??
    ...
    void on_DmdRadiusSlider_valueChanged(int value);
    void on_DmdSegStartSlider_valueChanged(int value);
    void on_DmdSegEndSlider_valueChanged(int value);
    void on_DmdKalmanSlider_valueChanged(int value);
    void on_DmdRefreshSlider_valueChanged(int value);
    void on_radioRealTime_toggled(bool checked);
    void on_radioSimulation_toggled(bool checked);
    void on_BtnLoadVideo_clicked();
    void on_BtnOneClickCalib_clicked();
    void on_comboTestVideo_currentIndexChanged(int index);
    void on_TargetPredictiveComp_valueChanged(int value);
